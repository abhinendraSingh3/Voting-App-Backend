...
const singleCandidate = async (req, res) => {
    try {
        const userId = req.data.userId;//coming form auth
        const candidateId = req.params.candidateId;//coming form url

        const candidateData = await candidateSch.findOne({ _id: candidateId }).select("-votes -voteCount")
        if (!candidateData) {
            return res.status(404).json({
                success: false,
                message: "No candidate Found"
